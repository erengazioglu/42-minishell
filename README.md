...
Minishell is a beast of a program that's composed of many different moving parts that have to play nice with each other and handle a multitude of edge cases. Its functionality is based on the `bash --posix` shell. 

It handles things as simple as:
```bash
...
But also crazy things like:
```bash
minishell> export a="ls" b="  -la"
minishell> $a$b
```
All while not crashing into pieces.
...
A `readline` loop that receives input, processes it, creates resources to be handled further ahead, and hands them over to the dispatcher if the AST is valid.

#### Tokenizer

The tokenizer (aka lexer) is responsible of taking a string input, and turning it into appropriate tokens, returning them as a linked list.

It can also take an existing token as a parameter, in which case the newly parsed tokens will be appended to the end of the existing ones. This is particularly useful for the unterminated pipe (`|`) character case, where the user is prompted for additional tokens. Example:
...
The role of the parser is to take a list of tokens generated by the [tokenizer](#tokenizer), and generate an abstract syntax tree in a recursive manner. It is responsible for handling syntax errors and freeing the tokens as necessary, guaranteeing a clean exit in case of issues with the input.

The AST tree is composed of:
- Leaves (representing commands),
- Nodes (representing pipes),
- Errors (representing a parsing issue).

These three different types of AST elements are grouped into a union called "AST", sharing the "type" as a common property to distinguish between them.

#### Dispatcher

After parsing, a valid AST is passed to a "dispatcher" that determines what to do with it. It's a huge chunk with interdependent parts, redirecting input/output, forking children, managing memory and arguments, handling IPC (interprocess communication), among many others.
